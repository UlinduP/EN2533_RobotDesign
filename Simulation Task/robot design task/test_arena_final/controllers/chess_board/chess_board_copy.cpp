...
//
  ...
  while (robot->step(TIME_STEP) != -1)
  {
    ...
    rotational_motor->setPosition(0.0);
    robot->step(1000);
    if (king_angle<M_PI*(3/2)){
      turnRobot(-90);
      //cout<<laser_sensor->getValue()<<endl;
    }
    else if (king_angle<M_PI*2)
    {
      turnRobot(90);  
    }
    robot->step(500);
    ...
    {
      turnRobot(-90);
      rotational_motor->setPosition(-M_PI/2);
      robot->step(1000);
      detectObjectBeforeKing(270);
      if (success)
      {
      ...
      {
        turnRobot(90);
        rotational_motor->setPosition(M_PI);
        linear_motor->setPosition(0.6);
        robot->step(1000);
        goToKing();
        if (king_row_found)
    ...
    else if (!king_row_found)
    {
      turnRobot(180);
      robot->step(1000);
      moveForward(1,distance_travelled_towards_king);
      turnRobot(90);
              rotational_motor->setPosition(M_PI);
        linear_motor->setPosition(0.6);
        robot->step(1000);
...
int moveForward(float speed, float distance = -1)  // 0.75 in scale is 15 in sensor val
{
  ...
  motors[2]->setVelocity(speed*MAX_SPEED);
  motors[3]->setVelocity(speed*MAX_SPEED);
...
int moveForwardDist(float speed)  // 0.75 in scale is 15 in sensor val
{
  ...
  while (robot->step(TIME_STEP) != -1){
    ...
    else
    {
      ...
      motors[2]->setVelocity(0);
      motors[3]->setVelocity(0);
      cout<<king_color_detector->getValue()<<endl;
      if (king_color_detector->getValue() < 512)
      {
        cout<<"White King Found!"<<endl;
        distance_travelled = right_front_wheel_pos_sensor->getValue() - startPos;
        turnRobot(180);
        moveForward(1, distance_travelled);
        boxDown();
      }
      else
      {
        cout<<"Black King Found!"<<endl;
        distance_travelled = right_front_wheel_pos_sensor->getValue() - startPos;
        turnRobot(180);
        moveForward(1, distance_travelled);
        turnRobot(90);
        moveForward(1,distance_travelled_towards_king);
        detectKing(M_PI+king_angle);
