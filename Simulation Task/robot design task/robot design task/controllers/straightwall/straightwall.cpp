...
int main(int argc, char **argv){
  ...
  while (robot->step(TIME_STEP) != -1) {
    ...
    for (int i = 0; i < NUM_IR_SENSORS; i++) {
      std::cout<<irSensors[i]->getValue()<<std::endl; 
    ...
    if (irValueFound != irSensorValues + 6) {
      ...
      double l_speed = 2.0 - correction;
      double r_speed = 2.0 + correction;
      if (l_speed < 0.0) {l_speed = 0.0;}
      if (l_speed > 4.0) {l_speed = 4.0;}
      if (r_speed < 0.0) {r_speed = 0.0;}
      if (r_speed > 4.0) {r_speed = 4.0;}
     ...
     // }
      ...
      double l_speed = 2.0 - correction; //5
      double r_speed = 2.0 + correction;
      if (l_speed < 0.0) {l_speed = 0.0;}
      if (l_speed > 4.0) {l_speed = 4.0;} //10
      if (r_speed < 0.0) {r_speed = 0.0;}
      if (r_speed > 4.0) {r_speed = 4.0;}
      motors[0]->setVelocity(r_speed);
      motors[1]->setVelocity(r_speed);
